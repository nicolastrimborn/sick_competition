...
def processPointCloud2(msg):
    ...
    slopecalc(cld[vmin:vmax, umin:umax]['x'], cld[vmin:vmax, umin:umax]['y']*-1, sensorpos)
    smoothness(cld[vmin:vmax, umin:umax]['z'])

    cld = cld[cld['intensity'] > 0]
    x0= cld['x'].ravel()
    y0= cld['y'].ravel()
    z0= cld['z'].ravel()
